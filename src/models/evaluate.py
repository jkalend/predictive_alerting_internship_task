...
def evaluate_file(pred_path: str | Path, recall_target: float = 0.80) -> pd.DataFrame:
    ...
    # ---- optimal for >= recall_target ----
    cands = df[df["recall"] >= recall_target]
    if not cands.empty:
        best = cands.loc[cands["precision"].idxmax()]
        print(f"\n  Best threshold for >={recall_target*100:.0f}% recall: {best.threshold:.4f}")
        _print_row(best)
    else:
        print(f"\n  [warn] No threshold achieves >={recall_target*100:.0f}% recall.")
